...
Author: Akira Adeniran-Lowe (akira.miranda.al@gmail.com)
...
from pointcept.datasets import build_dataset, collate_fn as pc_collate_fn
...
from .adapters import build_otta  
...
class AdapterBase:
    ...
    def build_data_iterator(self):
        ...
        for i in range(len(dataset)):
            full_frame_name = dataset.get_data_name(i)
            
            try:
                scene_prefix = '_'.join(full_frame_name.split('_')[:-1])
                if not scene_prefix:
                    raise IndexError
            except IndexError:

                scene_prefix = full_frame_name

            scene_data_indices[scene_prefix].append(i)
        
        scene_names = sorted(scene_data_indices.keys())
        self.logger.info(f"Found {len(scene_names)} unique scenes to process.")

        for scene_name in scene_names:
            indices = scene_data_indices[scene_name]
            self.logger.info(f"Scene '{scene_name}' contains {len(indices)} frames.")

            scene_subset = torch.utils.data.Subset(dataset, indices)
...
class SemSegTester(AdapterBase):
    ...
    def test(self):
        ...
        self.adapter = build_otta(self.cfg)
        self.adapter.attach(self.model)

        save_path = os.path.join(self.cfg.save_path, "result")
        make_dirs(save_path)
        # (unchanged) submission folder setup...
        if (
        ...
        for scene_idx, (scene_name, scene_loader) in enumerate(self.build_data_iterator()):
            logger.info(f"===== Scene [{scene_idx+1}]/{'?' } : {scene_name} =====")

            # we keep local counters for pretty logs
            num_items_in_scene = len(scene_loader.dataset)
            processed_in_scene = 0

            for local_idx, data_dict in enumerate(scene_loader):
                start = time.time()
                # identity collate => list of length N; we still process 1-by-1
                # to preserve the original assumption and logging granularity
                assert len(data_dict) == 1, "Expect batch size == 1 with identity collate"
                ...
                if os.path.isfile(pred_save_path):
                    logger.info(
                        f"{processed_in_scene+1}/{num_items_in_scene} (scene {scene_name}): {data_name}, loaded pred and label."
                    )
                    ...
                    for i in range(len(fragment_list)):
                        ...
                        input_dict = pc_collate_fn(fragment_list[s_i:e_i])
                        for key in input_dict.keys():
                            if isinstance(input_dict[key], torch.Tensor):
                                input_dict[key] = input_dict[key].cuda(non_blocking=True)
                        idx_part = input_dict["index"]

                        self.adapter.adapt(input_dict)
                        ...
                        logger.info(
                            "Test: {} [{}/{}]-{} | Scene: {} | Frag {}/{}".format(
                                data_name,
                                processed_in_scene + 1,
                                num_items_in_scene,
                                segment.size,
                                scene_name,
                                i,
                ...
                # (unchanged) submission writing per dataset type...
                if self.cfg.data.test.type in ["ScanNetDataset", "ScanNet200Dataset"]:
                    ...
                    np.array(pred + 1).astype(np.uint8).tofile(
                        os.path.join(
                            save_path, "submit", "lidarseg", "test", f"{data_name}_lidarseg.bin"
                        )
                    )
        ...
        # everything below stays the same (sync + final logs)...
        logger.info("Syncing ...")
        comm.synchronize()
