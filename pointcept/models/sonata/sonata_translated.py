...
class ProjHead(nn.Module):
    """Projection head with explicit input dimension."""

    def __init__(self, in_dim, out_dim=512, hidden=1024, bn=False, drop=0.0):
        super().__init__()
...
class TranslatorMLP(nn.Module):
    ...
    def __init__(self, dim, hidden=None, layers=2, dropout=0.0):
        super().__init__()
        ...
        for i in range(layers - 1):
            blocks += [
                nn.Linear(dim if i == 0 else hidden, hidden),
                nn.GELU(),
                nn.Dropout(dropout),
            ]
        blocks.append(nn.Linear(hidden if layers > 1 else dim, dim))
...
class ProtoAssign(nn.Module):
    """Soft prototype distribution for soft weights."""

    def __init__(self, in_channels, hidden=4096, embed=512, num_proto=4096, temp=0.07):
        super().__init__()
        self.m1 = nn.Linear(in_channels, hidden)
        self.act = nn.GELU()
...
class SonataTranslated(PointModel):
    """
    Dual-encoder + MLP translator training (no text). Works with paired or unpaired batches.
    ...
    def __init__(
        ...
        # Projectors
        self.ego_head = ProjHead(head_in_channels, out_dim=proj_out_dim, hidden=proj_hidden)
        self.exo_head = ProjHead(head_in_channels, out_dim=proj_out_dim, hidden=proj_hidden)
